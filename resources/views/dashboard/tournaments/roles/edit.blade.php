...
<div class="max-w-4xl mx-auto">
    ...
    <form method="POST" action="{{ route('dashboard.tournaments.roles.update', [$tournament, $role]) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="description"
                          ...
                          class="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-fuchsia-500 focus:border-transparent">{{ old('description', $role->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div class="space-y-4">
                @foreach($resources as $resource => $label)
                    @php
                        $currentPermission = old('permissions.'.$resource, $currentPermissions[$resource] ?? 'none');
                    @endphp
                        ...
                        <div class="flex space-x-4">
                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="radio"
                                       ...
                                       value="none"
                                       {{ $currentPermission === 'none' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="radio"
                                       name="permissions[{{ $resource }}]"
                                       value="view"
                                       {{ $currentPermission === 'view' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="radio"
                                       name="permissions[{{ $resource }}]"
                                       value="edit"
                                       {{ $currentPermission === 'edit' ? 'checked' : '' }}
            ...
            <div class="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
                <div class="flex items-start space-x-3">
                    ...
                    <p class="text-sm text-amber-400">
                        This role is currently assigned to {{ $staffCount }} {{ Str::plural('staff member', $staffCount) }}. Changes to permissions will affect their access immediately.
                    </p>
                </div>
            </div>
        @endif
