@php
    ...
    // Get user permissions if in dashboard
    $userPermissions = [];
    if ($isDashboard && auth()->check()) {
        $userPermissions = $tournament->getUserPermissions(auth()->user());
    }
    ...
    // Filter tabs based on permissions when in dashboard
    if ($isDashboard && auth()->check()) {
        $tabs = array_filter($tabs, function($tab) use ($userPermissions) {
            if (!isset($tab['resource'])) {
                return true;
            }

            // Check if user has at least view permission for this resource
            return isset($userPermissions[$tab['resource']]) && $userPermissions[$tab['resource']] !== 'none';
        });
    }
@endphp
